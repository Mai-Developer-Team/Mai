...
class UserSettingButton(miru.View):
    ...
    async def setting_nothing(self, button: miru.Button, ctx: miru.ViewContext):
        ...
        emb = hikari.Embed(
            ...
            description="Оповещения нужны для **получения буста, нового уровня, об заходе человека из черного списка и успешном апе на BotiCord**. Изначально они выключены, но рекомендуем включить",
            color=setting.color
        )
        emb.set_footer(
